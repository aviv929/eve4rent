...
namespace everything4rent
{
    ...
    {//Data Source=(LocalDB)\MSSQLLocalDB;AttachDbFilename=C:\Users\aviv9_000\Desktop\everything4rent\everything4rent\Database1.mdf;Integrated Security=True
        ...
        public static void add(string username, string password, string email, string name, string lastname, string birthdate,Image image, string paypal)
        {
            ...
            string qry = "INSERT INTO Users (username, [password], email,name,lastname,birthdate,picture,paypal) VALUES('"+username+ "', '"+password+ "', '"+email+ "', '"+name+ "', '"+lastname+ "', '"+birthdate+ "','@photo', '" + paypal+"'); ";
            con = new SqlConnection(cs);
        ...
        public static Image getImage(string username)
        {
            ...
            SqlCommand cmd;
            SqlDataReader dataReader;
            string qry = "select [picture] from Users where username= '" + username + "';";
            con = new SqlConnection(cs);
            ...
            {
                dataReader.Read();
                ans = (Image)dataReader.GetValue(0);
            }
